...
import { AnalogClock, getHandAngles } from "./analog-clock";
...
export default function ClockTutorial() {
  ...
  return <section className="clock-tutorial" aria-label={text.tutorialTitle}>
    ...
    <div className="tutorial-dial">
      <AnalogClock time={time} handAngles={getHandAngles(time)} isAnimating={false} isLocked={false} scoreVisualization={null} onTimeChange={changeTime} onTick={() => {}} />
    </div>
    ...
    <button className="tutorial-reset" type="button" onClick={() => { setTime(9 * 60); setTouched(false); }}>{text.tutorialReset}</button>
  </section>;
}
